...
class Wakka
{
    ...
    public function Link($tag, $method = '', $text = '', $track = 1)
    {
        ...
        elseif (ord($tag[0]) > 123) {
            ...
            } elseif ($this->HasAccess('create')) {
                return '<span class="missingpage">' . $text . '</span><a href="' . $this->Href('edit', $tag) . '">?</a>';
            }
  

            return '<span class="missingpage">' . $text . '</span>';
    ...
    {
        return $this->LoadAll('SELECT uname FROM ' . $this->db->prefix('users') . ' WHERE level > 0 ORDER BY uname');
    }
    ...
    public function HasAccess($privilege, $tag = '', $user = '')
    {
        ...
        if ($this->xoopsConfig['isadmin']) {
            return true;
        }

        if ('create' == $privilege) {
            if (0 == count($this->xoopsConfig['writepem'])) {
                return true;
            }  

            foreach ($this->xoopsConfig['groups'] as $groupid) {
                if (in_array($groupid, $this->xoopsConfig['createpem'], true)) {
                    return true;
                }
            }
        }
        ...
        if ('*' == $acl['list']) {
            if ('write' == $privilege) {
                ...
                foreach ($this->xoopsConfig['groups'] as $groupid) {
                    if (in_array($groupid, $this->xoopsConfig['writepem'], true)) {
                        return true;
                    }
                }
            } elseif ('comment' == $privilege) {
                if (0 == count($this->xoopsConfig['commentspem'])) {
                    return true;
                }  

                foreach ($this->xoopsConfig['groups'] as $groupid) {
                    if (in_array($groupid, $this->xoopsConfig['commentspem'], true)) {
                        return true;
                    }
                }
            } else {
    ...
    public function Run($tag, $method = '')
    {
        ...
        if (preg_match("/^index\.php/", $tag)) {
            header('Location: ' . XOOPS_URL . '/modules/wakka/' . $this->config['root_page'] . '?story=1');
        } elseif (!preg_match("/\.xml$/", $this->method)) {
